...
| 9 | Notifications | Desktop (macOS), Discord, Telegram, Slack, and email alerts for high-value trends |
...
└── notification_system/
    ...
    ├── slack.py                   # Slack webhook or bot API
...
| `USE_LOCAL_EMBEDDINGS` | Similarity | `true` uses local `all-MiniLM-L6-v2`; `false` uses `OPENAI_EMBEDDING_MODEL`. Creator and content vectors always use this same setting |
...
| `NOTIFY_DESKTOP_ENABLED` / `NOTIFY_DISCORD_ENABLED` / `NOTIFY_TELEGRAM_ENABLED` / `NOTIFY_EMAIL_ENABLED` / `NOTIFY_SLACK_ENABLED` | Notifications | Toggle each channel independently |
...
| `SLACK_WEBHOOK_URL` or `SLACK_BOT_TOKEN`, `SLACK_CHANNEL` | Slack notifications | Configure an incoming webhook, or a bot token and destination channel |
...
# Use the MongoDB `_id` returned by the drafts endpoint, then approve (send) or reject
...
| POST | `/api/similarity/embed-pending` | Embed newly processed content; `force=true` re-embeds legacy data after a backend change |
...
| POST | `/api/content/regenerate/{content_id}` | Regenerate one of `titles`, `hooks`, `outline`, `script`, or `thumbnails` |
...
| POST | `/api/notify/test` | Send a test notification to `desktop`, `discord`, `telegram`, `email`, or `slack` |
| POST | `/api/feedback/trends/{trend_id}` | Record `accept`, `reject`, or `ignore` feedback; `trend_id` is the external `content_id` returned by `/api/trends` |
| GET | `/api/feedback/history/{creator_id}` | Get a creator's feedback history |
| GET | `/api/feedback/summary/{creator_id}` | Get feedback counts and category preferences |
...
- **Embedding compatibility**: creator profiles and content use the same `USE_LOCAL_EMBEDDINGS` setting. If you change it, rebuild the creator profile and call `/api/similarity/embed-pending?force=true&limit=1000` once so existing vectors are regenerated in the same space.
...
| Similarity logs incompatible dimensions/spaces | Embeddings were created with different backends; rebuild the creator profile, then call `/api/similarity/embed-pending?force=true&limit=1000` |
| Draft approval says `Invalid draft_id format` | Use the 24-character `_id` returned by `/api/emails/drafts`, not a list position or Gmail draft ID |
